...
import {
  ...
  ArrowUpOutlined, BorderOuterOutlined,
  CaretRightOutlined,
  InfoCircleOutlined,
  PauseOutlined
...
class SoundImage extends React.Component {
  ...
  componentDidMount() {
    ...
    for (const layer of this.props.layers) {
      const split = layer.src.split("/")
      const src = process.env.REACT_APP_FILE_SERVER + "/images/" + split[split.length - 1]; 
      ...
      layers[layerName].build(src, null, null, true, this.openCvRef.current, this.props.description);
    ...
    if (Object.keys(layers).length === 0) {
      ...
      this.hapticTools[Constants.IMAGE_TYPE_COMPOSITE_ONLY] = 
        new Haptic(this.props.soundEffects);
    } else {
